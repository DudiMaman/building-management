      ...
      <div className="container">
        <div className="grid items-end gap-6 md:grid-cols-12">
          <div className="md:col-span-7">
            <div className="eyebrow-he">תמחור</div>
            ...
            >
              <button
                ...
                className={`rounded px-4 py-2 text-[13px] font-semibold transition-colors ${
                  cycle === 'monthly' ? 'text-[var(--paper)]' : 'text-[var(--ink-2)]'
                }`}
                style={{ background: cycle === 'monthly' ? 'var(--ink)' : 'transparent' }}
              >
                חיוב חודשי
              </button>
              <button
                ...
                className={`rounded px-4 py-2 text-[13px] font-semibold transition-colors ${
                  cycle === 'annual' ? 'text-[var(--paper)]' : 'text-[var(--ink-2)]'
                }`}
                style={{ background: cycle === 'annual' ? 'var(--ink)' : 'transparent' }}
              >
                חיוב שנתי{' '}
                <span style={{ color: cycle === 'annual' ? 'var(--brass-2)' : 'var(--brass)' }}>
                  −15%
                </span>
              </button>
            </div>
...
function PriceCard({ tier, cycle }: { tier: Tier; cycle: Cycle }) {
  ...
  return (
    <div
      className={`relative flex flex-col rounded-lg border p-6 md:p-7 ${tier.emphasis ? 'shadow-lg' : ''}`}
      style={{
        background: tier.emphasis ? 'var(--ink)' : 'var(--paper)',
        borderColor: tier.emphasis ? 'var(--ink)' : 'var(--line)',
        color: tier.emphasis ? 'var(--paper)' : 'var(--ink)',
      }}
    >
      {tier.emphasis && (
        <span
          className="absolute -top-3 right-6 rounded-full px-3 py-1 text-[10px] font-bold tracking-wider uppercase"
      ...
      <h3 className="text-[20px] font-bold">{tier.name}</h3>
      <p
      ...
      <div className="mt-6 flex items-baseline gap-2">
        ...
        {display.small && (
          <span className="text-[12px]" style={{ color: tier.emphasis ? 'var(--bg-2)' : 'var(--ink-3)' }}>
            {display.small}
          </span>
        )}
      </div>
      ...
      >
        <li className="flex justify-between">
          <span>בניינים</span>
          <span className="font-bold" style={{ color: tier.emphasis ? 'var(--paper)' : 'var(--ink)' }}>
            {tier.buildings}
          </span>
        </li>
        <li className="flex justify-between">
        ...
        {tier.features.map((f) => (
          ...
          >
            <Check
              ...
              style={{ color: tier.emphasis ? 'var(--brass)' : 'var(--brass)' }}
            />
            <span>{f}</span>
          </li>
        ))}
      ...
      <Link
        href={tier.href}
        className="mt-7 inline-flex items-center justify-center gap-1.5 rounded-md py-3 text-[14px] font-bold transition-colors"
        style={{
...
function RoiCalculator() {
  ...
  return (
    <div
      className="mt-16 overflow-hidden rounded-lg border md:mt-20"
    ...
    >
      <div className="grid md:grid-cols-5">
        ...
        >
          <div className="eyebrow-he">מחשבון ROI</div>
        ...
        <div className="p-6 md:col-span-3 md:p-8" style={{ background: 'var(--paper)' }}>
          ...
          <div
            className="mt-6 grid grid-cols-3 gap-px overflow-hidden rounded-md border"
            style={{ borderColor: 'var(--line)', background: 'var(--line)' }}
          >
            <ResCell label="גבייה חודשית" value={`₪${calc.monthlyBilling.toLocaleString('he-IL')}`} />
            <ResCell label="צפי גביה נוספת" value={`+₪${Math.round(calc.recovered).toLocaleString('he-IL')}`} accent />
          ...
          >
            <div className="text-[11px] font-bold tracking-wider uppercase" style={{ color: 'var(--brass-2)' }}>
              NET MONTHLY GAIN
            </div>
...
function NumField({
  ...
  return (
    <label className="block">
      ...
      <div
        className="mt-1 flex items-baseline justify-between border-b py-1.5 ltr"
      ...
      >
        ...
        <input
          ...
          className="tnum w-full bg-transparent text-[24px] font-bold outline-none"
...
function ResCell({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="p-3" style={{ background: 'var(--paper)' }}>
      ...
      <div
        className="tnum mt-0.5 text-[17px] font-bold ltr"
